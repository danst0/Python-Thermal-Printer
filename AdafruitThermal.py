...
# This is pretty much a 1:1 direct Python port of the AdafruitThermal
...
# needed.  This should simplify porting existing AdafruitThermal-based
...
class AdafruitThermal(Serial):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print_density   = 14 # 120% (can go higher, but text gets fuzzy)
        print_break_time =  4 # 500 uS
    ...
    def print_barcode(self, text, type):
        ...
        super(AdafruitThermal, self).write(text)
        self.prev_byte = '\n'
        self.feed(2)
    ...
    def set_ascii_font_mode(self):
        self.set_print_mode(self.FONT_MASK)
    ...
    def feed_rows(self, rows):
        ...
        self.timeout_set(rows * dot_feed_time)
    ...
    def set_size(self, value):
        ...
        self.write_bytes(29, 33, size, 10)
    ...
    def print_bitmap(self, w, h, bitmap, LaaT=False):
        ...
        if LaaT: max_chunk_height = 1
        else:    max_chunk_height = 255

        i = 0
        for row_start in range(0, h, max_chunk_height):
            ...
            for y in range(chunk_height):
                for x in range(row_bytes_clipped):
                    super(AdafruitThermal, self).write(
                      chr(bitmap[i]))
                    i += 1
